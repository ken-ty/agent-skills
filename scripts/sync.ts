...
const GITIGNORE = path.join(REPO_SKILLS, ".gitignore");
...
function writeGitignore(names: string[]): void {
  const block = [
    BEGIN,
    "# 3rd-party skills: tracked in ../skills.lock, restored by `npm run sync`.",
    ...names.map((n) => `/${n}/`),
    END,
  ].join("\n");
 ...
 * download is due, so pointing at `npm run sync` would send you in a circle.
 */
function reportGitBacked(catalog: ReturnType<typeof readCatalog>, present: Set<string>): void {
  ...
  for (const name of missing) {
    console.error(`  ${name}: in ${tilde(REPO_CATALOG)} but missing on disk`);
...
function main(): void {
  ...
  if (uncatalogued.length > 0) {
    ...
    console.log("  add an entry so author / refs are recorded — `npm run doctor` lists them");
  }
}
