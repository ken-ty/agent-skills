...
入れたか」が追跡できず、新しいマシンで再現できない。このリポジトリを唯一の canonical source
にして、その両方を解決する。
...
```mermaid
flowchart BT
  subgraph repo["agent-skills (このリポジトリ)"]
    direction LR
    skills["skills/<br/><i>唯一の実体</i>"]
    lock["skills.lock<br/><i>取得 — CLI が書く</i>"]
    catalog["catalog.json<br/><i>来歴 — repo が書く</i>"]
  end
  ...
  agentsSkills ==>|"symlink (自作)"| skills
  agentsLock ==>|"symlink (自作)"| lock
  claude -->|"symlink (npx skills が生成)"| agentsSkills
  universal -.->|"直接読む"| agentsSkills
```

太線の 2 本がこのリポジトリの持ち物。`catalog.json` に矢印が無いのは、`~/.agents/` から
参照されない repo 専用ファイルだからで、欠落ではない。

自前で管理するのは `~/.agents/` 配下の **2 本の symlink だけ**。その下、各エージェントへの
ファンアウトは `skills` CLI の責務であり、再実装しない。
...
つまり `~/.agents/skills` をこのリポジトリへ向けるだけで、CLI が書き込むすべてが
自動的に版管理下に入る。取得も配布も書く必要がない。
...
`npm run doctor` はこの区別を踏まえて判定する。Codex の `~/.codex/skills` が空でも正常。
...
`npm run doctor` はエージェントディレクトリ内の実ディレクトリを検出するので、
...
一見きれいだが、採らなかった。誰も解決できない URL は `npm run sync` で取得できるという
...
- **削除しない** — 邪魔なものは `.bak-<timestamp>` にリネームして残す
- **`--dry-run`** — 実行前に全操作を表示できる
- **冪等** — 正しい symlink が既にあれば何もしない
- **既に別の場所を指す symlink があれば中断する** — 勝手に張り替えると、知らないストアを
  孤立させる可能性があるため、人間に判断させる
